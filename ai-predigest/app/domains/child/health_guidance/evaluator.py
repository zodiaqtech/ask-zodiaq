"""
Health guidance evaluator for child-related health matters.
...
class ChildHealthGuidanceEvaluator(BaseEvaluator):

    domain = "Child"
    subtopic = "Health Guidance"

    # ==================================================
    # MAIN EVALUATION (FINANCE-CLONE)
    # ==================================================
    def evaluate(
        self,
        ...
        **kwargs
        ...
        if house_config:
            ...
            logger.warning(f"No config for question, using fallback")
             # ✅ CHILD HEALTH FALLBACK (DOMAIN-CORRECT)
            primary_houses = {1, 5}
        ...
        sub_subdomain: str = kwargs.get("sub_subdomain", "Physical Growth")
        ...
        logger.info("=" * 80)
        logger.info("CHILD HEALTH GUIDANCE EVALUATOR (FINANCE-CLONE STRUCTURE)")
        logger.info("=" * 80)
    ...
    def get_questions(self) -> List[Question]:
        return [
            Question(
                id="CH_HLT_1",
                question="What guidance does astrology offer regarding physical and mental growth and health concerns?",
                meta=QueryMeta(QueryType.NON_TIMING, EventPolarity.NEUTRAL, InterpretationGoal.STATUS),
                sub_subdomain="Physical Growth"
            )
        ]
